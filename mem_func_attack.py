...
def shell_code_create(hdc, a2, a3, a4, a5):
    ...
    shellcode += b'\x68' + struct.pack('<I', in_a4 & 0xFFFFFFFF)
    # idk if we should be masking, maybe mask them all? my mask might be wrong
...
def Inject_shell_code(pm, a2,a3):
  ...
  #determining your computer setup you'll need to change the application path
    ...
    if not a3: a3 = 0
    offset = ((21 * a2) + a3) * 4
    address = 0x01008AB0 + offset
    bytes = pm.read_bytes(address, 4)  
    a4 = struct.unpack('i', bytes)[0]
    # address = 0x01008AB0
    #bytes = pm.read_bytes(address, ((21*a2)+a3)  * 4)
    #a4 = struct.unpack('i', bytes)[0]
